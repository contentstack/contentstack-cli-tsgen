...
async function tsgenRunner(outputFile, contentTypes, prefix = '', includeDocumentation = true) {
    ...
    const output = await format([
        builtins_1.defaultInterfaces(prefix).join('\n\n'),
        [...globalFields].join('\n\n'),
        definitions.join('\n\n'),
    ].join('\n\n'));
